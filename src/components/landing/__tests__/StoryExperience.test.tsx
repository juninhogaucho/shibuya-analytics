...
describe('StoryExperience', () => {
  test('renders the provisional fingerprint journey and routes to the public upload step', async () => {
    ...
    expect(screen.getByText('Public handoff packet')).toBeInTheDocument()
    expect(screen.getByText('What survives from story into upload.')).toBeInTheDocument()
    expect(screen.getByText('Secret-free routing context')).toBeInTheDocument()
    expect(screen.getByText('Stored at next click')).toBeInTheDocument()
    expect(screen.getByText('Public markers')).toBeInTheDocument()
    ...
    expect(screen.getByText(/the packet can route the next page and preserve the question/i)).toBeInTheDocument()
    ...
    expect(screen.getByText(/archetype=priya; axis=drawdown_pressure/i)).toBeInTheDocument()
    expect(screen.getByText(/story=guided; scene_count=1; pain_axes=drawdown_pressure/i)).toBeInTheDocument()
    expect(screen.getByText(/Mirror selected, Pain axis tapped/i)).toBeInTheDocument()
