...
describe('ResetProDemoCommandCenter', () => {
  test('renders a private demo path with explicit demo boundaries', () => {
    ...
    expect(screen.getByText('RESET PRO OPENING REEL')).toBeInTheDocument()
    expect(screen.getByText('Make the first private screen feel like a handoff film.')).toBeInTheDocument()
    expect(screen.getByText(/The workspace opens with receipt, question, one surface, and proof exit/i)).toBeInTheDocument()
    expect(screen.getByText('0:00 / Receipt burn-in')).toBeInTheDocument()
    expect(screen.getByText('Open on a cold sample warning before any metric appears.')).toBeInTheDocument()
    expect(screen.getByText('No public story, report, upload packet, or locked module is attached.')).toBeInTheDocument()
    expect(screen.getByText('0:25 / Question becomes mission')).toBeInTheDocument()
    expect(screen.getByText('Put the missing private question on screen before showing the command center.')).toBeInTheDocument()
    expect(screen.getByText('0:55 / One surface only')).toBeInTheDocument()
    expect(screen.getByText('Show one intervention surface instead of touring every card.')).toBeInTheDocument()
    expect(screen.getByText('1:40 / Exit through proof')).toBeInTheDocument()
    expect(screen.getByText('End at append proof while the viewer still wants the answer.')).toBeInTheDocument()
    expect(screen.getByText('No performance, pass-rate, profit, or drawdown-improvement claim is allowed.')).toBeInTheDocument()
  ...
  test('renders carried report context when the private demo came from a free report', () => {
    ...
    expect(screen.getByText('RESET PRO OPENING REEL')).toBeInTheDocument()
    expect(screen.getByText('Open on the carried report packet before any metric appears.')).toBeInTheDocument()
    expect(screen.getByText('Report free-report-123; evidence Sample history packet; market route preserved.')).toBeInTheDocument()
    expect(screen.getByText('Put the private question on screen: Does the trader become a different operator near the drawdown line?')).toBeInTheDocument()
    expect(screen.getByText('Bridge card first, Mission HQ second.')).toBeInTheDocument()
    expect(screen.getByText('Choose the surface that best tests the carried question.')).toBeInTheDocument()
    expect(screen.getByText('RESET PRO LIVING MIRROR')).toBeInTheDocument()
    ...
    expect(screen.getByText('RESET PRO WORKSPACE STATUS SNAPSHOT')).toBeInTheDocument()
    expect(screen.getByText('RESET PRO PRIVATE GATE CHECKSUM')).toBeInTheDocument()
    expect(screen.getByText('Attached after presenter unlock')).toBeInTheDocument()
