...
describe('PrivateDemoPage', () => {
  ...
  test('unlocks reset pro sample preview only with the configured presenter code', async () => {
    ...
    expect(screen.getByText('Private demo gate sequence')).toBeInTheDocument()
    expect(screen.getByText('Unlock Reset Pro like an evidence handoff, not a password wall.')).toBeInTheDocument()
    expect(screen.getByText('Prove the private question arrived.')).toBeInTheDocument()
    expect(screen.getByText('Separate demo structure from live proof.')).toBeInTheDocument()
    expect(screen.getByText('Show what the unlock will carry.')).toBeInTheDocument()
    expect(screen.getByText('Enter Reset Pro with one job.')).toBeInTheDocument()
    expect(screen.getByText('Private demo claim script')).toBeInTheDocument()
    expect(screen.getByText('Do not claim live activation, backend normalization, or account-specific improvement.')).toBeInTheDocument()
