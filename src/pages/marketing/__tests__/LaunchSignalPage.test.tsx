...
describe('LaunchSignalPage', () => {
  test('routes the shared signal entry through the canonical story-first demo packet', () => {
    render(
      <MemoryRouter initialEntries={['/signal?market=global']}>
        <LaunchSignalPage />
      </MemoryRouter>,
    )

    expect(screen.getByText('Shibuya demo launcher')).toBeInTheDocument()
    expect(screen.getByText(/This shared link is a truthful demo path/i)).toBeInTheDocument()
    ...
    expect(presenterGate).not.toHaveAttribute('href', '/private-demo?market=global')
  })
})
