...
describe('ifx demo journey', () => {
  ...
  test('exports the canonical three-minute operator run sheet', () => {
    expect(IFX_DEMO_OPERATOR_RUNBOOK).toHaveLength(5)
    expect(IFX_DEMO_OPERATOR_RUNBOOK.map((beat) => beat.title)).toEqual([
      'Open with the wedge',
      'Name the mirror',
    ...
    expect(IFX_DEMO_OPERATOR_RUNBOOK[0].say).toContain('Every provider sells dashboards')
    expect(IFX_DEMO_OPERATOR_RUNBOOK[2].boundary).toContain('Sample packet proves demo continuity only')
    expect(IFX_DEMO_OPERATOR_RUNBOOK[4].boundary).toContain('No improvement claim')
  })
